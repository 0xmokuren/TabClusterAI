...
> **ヒント:** モデルのダウンロードは任意です。ポップアップの **「起動時にローカルモデルを自動ダウンロード」** は既定でオフのため、起動しただけで容量を消費することはありません。API だけを使う場合はオフのままにしてください。オンにすると、ポップアップを開いた時点で事前ダウンロードを開始します。
...
**オンデバイスモードのみ**が対象です。インストール直後は勝手にダウンロードせず、明示的に「モデルをダウンロード」ボタンを押したときだけ開始します。

| 段階 | 何が起きるか | 画面表示 |
| --- | --- | --- |
| **1. トリガー** | ポップアップの「**モデルをダウンロード**」ボタンを押す | 「ダウンロードを開始しています…」 |
| **2. ダウンロード** | Chrome が約 22 GB 取得 | パーセント + 推定容量 |
| **3. バックグラウンド** | 事前 DL している場合あり | 経過時間 |
| **4. 読み込み** | メモリへ展開 | 「モデルを読み込んでいます…」 |
| **5. 完了** | 以降はキャッシュ利用、Analyze ボタンが有効化 | 「ダウンロードが完了しました」 |
...
> **ヒント:** 進捗表示のためポップアップを**開いたまま**待ってください。閉じても Chrome 側の DL は続く場合がありますが、UI の進捗は止まります。

> **Gemini API だけ使う場合:** 「モデルをダウンロード」ボタンを押さなければローカルモデルは取得されません。プロバイダーを Gemini API に切り替えてご利用ください。

### モデルの削除

ダウンロード済みのモデルを削除したい場合は、ポップアップの「**Chrome の設定でモデルを管理**」ボタンから `chrome://on-device-internals` を開き、Chrome の管理画面で手動削除してください。Chrome の `LanguageModel` API には削除メソッドがないため、拡張機能から直接削除はできません。なお、空きディスクが 10GB を切ると Chrome が自動的にモデルを削除する仕様もあります。
...
<sub>Tab Cluster AI v1.5.5 · <a href="https://github.com/0xmokuren/TabClusterAI/issues">Issue を報告</a> · <a href="README.md">English</a></sub>
